...
public:
    ...
    BFS(int rows, int columns, vector<pair<int, int>> *visitedNodes, set<pair<int, int>> *walls)
    {
        this->rows = rows;
...
template <typename T>
void BFS<T>::printPairs(vector<pair<T, T>> const &input)
{
    int n = input.size();
...
template <typename T>
bool BFS<T>::isValid(int x, int y, int N, set<pair<int, int>> &walls)
{
...
template <typename T>
void BFS<T>::findPath(Node *node, vector<pair<int, int>> &path)
{
    if (node != nullptr)
    {
...
template <typename T>
vector<pair<int, int>> BFS<T>::findPath(int matrixSize, int x, int y, int endX, int endY, int foodX, int foodY, std::function<void()> renderScreenCallback)
{
    ...
    while (!q.empty())
    {
        ...
        if (i == foodX && j == foodY && foodLeft == 1)
        {
            foodLeft = 0;
        }

        if (i == endX && j == endY && foodLeft == 0)
        {
            findPath(curr, path);
            return path;
        }
